...
if check_password():
    ...
    if dift>0:
        while  crf>=tol | inter<100:
            ...
            #with col1:
            var1=st.selectbox('Select the variable 1:', ['P(bar)','DP(Bar)','DP(m)','Velocity(m/s)','Elem DP (m)'],var1)
            #with col2:
            var2=st.selectbox('Select the variable 2:', ['Velocity(m/s)','DP(Bar)','DP(m)','P(bar)','Elem DP (m)'],var2)
